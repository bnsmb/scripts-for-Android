...
#h#               [--checkonly] [--check] [--singlestep] [--singlestep_on_error] [--unique] [--trace] [--info] [--print_includefile_help] 
#h#               [-i|--includefile [?|+]filename]  [--no_init_tasks[ [--no_finish_tasks] [--only_list_tasks] [--disabled_tasks task1[...,task#]] 
#h#               [--list_disabled_tasks] [--enable_all_tasks]
...
#H#
#H#        The task names can be used with or without the leading \"task_\" if nothing else is stated below.
...
#H#
#H#        Use the parameter \"--print_task_template\" to generate a template for a new task; use the parameter \"--create_include_file_template\" 
#H#        to generate a new (empty) include file.
#H#
#H#
#H#        Patterns
#H#        --------
#H#
#H#        You can also use pattern with \"*\" and \"?\" -- e.g use the parameter \"check*\" to execute all tasks beginning with \"check\".
#H#
#H#
#H#        Task groups
#H#        -----------
...
#H#        The use of the variable \"DEFAULT_TASKS\" is also necessary to change the order for executing the tasks.
...
#H#        Use the parameter \"ALL\" only if really necessary -- in principle you should never use this parameter.
#H#
#H#
#H#        Disable tasks
#H#        -------------
...
#H#        To list all disabled tasks use the parameter \"--list_disabled_tasks\".
#H#        To temporary enable the disabled tasks use the parameter \"--enable_all_tasks\".
#H#
#H#
...
#H#        Note:
#H#
#H#        Additional tasks that should not be executed if the parameter "all" is used can now be defined in 
#H#        the function init_vars in the variable NO_DEFAULT_TASKS
#H#
#H#
#H#
...
#H#        These parameter will be ignored by the script ${REAL_SCRIPTNAME##*/}. 
#H#
#H#   --includefile
...
#H#        To read additional include files after reading the default include file add a leading plus sign \"+\" to the 
#H#        name of the includefile
...
#H#        Do not use the leading string \"task_\" in the patterns.
#H#   --list_tasks
...
#H#   --list_disabled_tasks
#H#       - execute the function init_tasks if defined, list the disabled tasks, and exit
...
#H#  --singlestep_on_error
#H#      - enable single step mode after a task ends with an error
#H#   --unique
...
#H#
#H#   --enable_all_tasks
#H#        enable all disabled tasks again 
#H#
...
#H#        Set the environment variable NO_OF_LOGFILES_TO_KEEP to change this.
...
#V#
#V#   20.10.2020 v2.0.1 /bs
#V#     the script now also writes the template for a task function to the output file if the 
#V#       parameter --create_include_file_template is used
#V#
#V#   18.11.2020 v2.0.2 /bs
#V#     fixed a bug in the handling for disabled tasks 
#V#
#V#   19.11.2020 v2.1.0 /bs
#V#     added the parameter --enable_all_tasks
#V#     added the parameter --list_disabled_tasks
#V#
#V#   19.11.2020 v2.1.1 /bs
#V#     fix some typos in the usage help
#V#
#V#   18.12.2020 v2.1.2 /bs
#V#     there was an error in processing the parameter for init task:
#V#       the parameter processing only worked if called with task parameter
#V#
#V#   31.01.2021 v2.1.3 /bs
#V#     the script did not allow "/" in task parameters -- fixed
#V#
#V#   01.05.2021 v2.1.4 /bs
#V#     added the variable NO_OF_LOGFILES_TO_KEEP to define the number of old logfiles to keep
#V#     the variables TMPFILE1 and TMPFILE2 were only filled if running under Linux -- fixed
#V#     added the variable SED
#V#     the function show_extended_usage_help (executed if the parameter "-v -h" are used) now
#V#       prints more details for the supported environment variables
#V#     the script now prints a hint if the equal sign (=) is used in an unknown parameter
#V#
#V#   05.08.2021 v2.1.5 /bs
#V#     added the function gettime_in_seconds ("date +%s" is not supported on all OS)
#V#
#V#   25.08.2021  v2.2.0 /bs
#V#     corrected a minor bug in the function ListDefaultTasks (NO_OF_TASKS_DEFINED was not a local variable)
#V#     additional tasks that should not be executed if the parameter "all" is used can now be defined in 
#V#       the function init_vars in the variable NO_DEFAULT_TASKS
#V#     added the function tryIncludeScript
#V#     added the function includeScript
#V#
#V#   01.01.2023  v2.2.1 /bs
#V#     the leading plus signs "++" where missing in the code to check the  parameter ++abort_on_error  -- fixed
#V#
#V#   21.01.2023  v2.2.2 /bs
#V#     the usage help printed with "-h" was missing one line -- fixed
#V#
#V#   08.10.2023  v2.2.3 /bs
#V#     added the syntax with the leading plus sign \"+\" (--includfile +filename)
#V#
#V#   22.01.2024  v2.2.4 /bs
#V#     grep -E is not supported by the grep from Solaris 10 -- fixed (-> use the variable EGREP to execute grep)
#V#     use gsed instead of sed if available (necessary for Solaris 10) (-> use the variable SED to execute sed)
#V#
#V#   27.04.2024  v2.2.5 /bs
#V#     the script now prints an empty line before the question "Execute the task ... " in single step mode
#V#
#V#   05.05.2024  v2.3.0 /bs
#V#     added the parameter --singlestep_on_error to enable single step mode only if a task ends with an erro
#V#
#V#   06.05.2024  v2.3.1 /bs
#V#     added the alias "--single-step" for the parameter "-singlestep"
#V#
#V#   08.05.2024  v2.3.2 /bs
#V#     minor bug and typo fixes in the messages
#V#

#V#   19.05.2024  v2.3.3 /bs
#V#     added commands to enable single-step while in the DebugShell
#V#     the name of the task in the single-step prompt is now without leading "task_"
...
#T#     to a function if necessary and "typeset -f" is supported by the shell used
...
#
# a workaround is necessary to get the device/file used for STDOUT and STDIN in some circumstances
#        
...
# stty settings (used in die to reset the stty settings if necessary)
#
__STTY_SETTINGS=""
...
#
# note: LogMsg calls the function RotateLog if necessary
#
function LogMsg {
...
"        ;;
      ...
      "singlestep" | "single-step" | "ss" | "s" )
        ...
        "printf" "Single step is now enabeld \n"        
        ;;

      "singlestep_on_error" | "single-step_on_error" | "se" )
        SINGLE_STEP_MODE=${__FALSE}
        ENABLE_SINGLE_STEP_ON_ERROR=${__TRUE}
        "printf" "Single step will be enabed automatically after the next failed task \n"        
        ;;
...
# check_rcm_values
#
# init the variables necessary for RCM access
#
...
#
# note: the RCM_USERID is NOT necessary for dbquery and dbgetfile
#
# This function is only useful in RCM environments!
#
...
#  HELPMSG="$( grep "^#H#" "${REAL_SCRIPTNAME}" | cut -c4- | ${SED} "s/sync_install_server_directory/${SCRIPTNAME}/g" )"
...
# - use "die [returncode] [message]" to end the script if necessary
#
...
#
#  add tasks that should not be executed if the parameter all is used to the variable NO_DEFAULT_TASKS
#
# NO_DEFAULT_TASKS=""
...
RE_ENABLE_DISABLED_TASKS=${__FALSE}
...
while [ $# -ne 0 ] ; do
  case $1 in
    ...
    --singlestep_on_error | --single-step_on_error )
      ENABLE_SINGLE_STEP_ON_ERROR=${__TRUE}
      ;;
...
#      ;;
    ...
    --enable_all_tasks )
      RE_ENABLE_DISABLED_TASKS=${__TRUE}
      ;;
...
if [ ${CREATE_INCLUDE_FILE_TEMPLATE} = ${__TRUE} ] ; then
  ...
  if [ $? -eq 0 ] ; then
    ...
    echo "${INCLUDE_FILE_TEMPLATE}
${TASK_FUNCTION_TEMPLATE}"> "${NEW_INCLUDE_FILE}"      
    if [ $? -ne 0 ] ; then
...
#
if [ $# -eq 0 -a ${LIST_DISABLED_TASKS} = ${__FALSE} ] ; then
  LogMsg "-"
...
# ------------------------------------------------------------------------
  ...
  while [ $j -lt ${TASKS_FOUND[0]} ] ; do
    ...
    if [[ ${TASK_GROUPS} = *\ ${CUR_TASK}\ * && ${CUR_TASK} != *\** && ${CUR_TASK} != *\?* ]] ; then
...
if typeset +f init_tasks >/dev/null ; then
  ...
  if [ ${ONLY_LIST_TASKS_TO_EXECUTE} = ${__FALSE} ] ; then
    if [ ${DO_NOT_EXECUTE_INIT_TASKS} = ${__TRUE} ] ; then
      ...
      TEMPVAR="$( echo "${NO_DEFAULT_TASKS}" | ${EGREP} -v "^$|^#" )"
      NO_DEFAULT_TASKS=""
      for CUR_VAR in ${TEMPVAR} ; do 
        NO_DEFAULT_TASKS="${NO_DEFAULT_TASKS} ${CUR_VAR}"
      done

      if [ "${NO_DEFAULT_TASKS}"x != ""x ] ; then
        LogMsg "Additional tasks are disabled in the function \"init_tasks\": "
   
        LogMsg "-" "$( echo "${NO_DEFAULT_TASKS}" | tr " " "\n"  ;echo " " )"
...
if [ ${LIST_DISABLED_TASKS} = ${__TRUE} ] ; then
  
  if [ "${DISABLED_TASKS}"x != ""x ] ; then
    LogMsg "Disabled tasks are: "
    LogMsg "-" "$( echo "${DISABLED_TASKS}" | tr -s " "  | tr " " "\n" )"
    LogMsg "-"
  else
    LogMsg "There are no disabled tasks"
  fi

  if [ "${DISABLED_TASKS_FOUND_IN_THE_PARAMETER}"x  != ""x ] ; then
    LogMsg "Disabled tasks in the parameter are: "
    LogMsg "-" "$( echo "${DISABLED_TASKS_FOUND_IN_THE_PARAMETER}" | tr -s " "  | tr " " "\n" )"
    LogMsg "-"
  else
    LogMsg "There are no disabled tasks in the parameter"
  fi
  LogMsg "-" "${TASK_SEPARATOR_LINE}"
...
if [ ${SINGLE_STEP_MODE} = ${__TRUE} ] ; then
  ...
  if [ ${ENABLE_SINGLE_STEP_ON_ERROR} = ${__TRUE} ] ; then
    LogWarning "Enabling single step mode after task ends with an error is disabled in single-step mode"
  fi

  ENABLE_SINGLE_STEP_ON_ERROR=${__FALSE}
  
  LogMsg "-"
elif [ ${ENABLE_SINGLE_STEP_ON_ERROR} = ${__TRUE} ] ; then
  LogMsg "The remaining tasks will be executed in single step mode after a task ends with an error"
  LogMsg "-"
fi
...
#
  if [ "${DISABLED_TASKS}"x != ""x ] ; then
    if [ ${RE_ENABLE_DISABLED_TASKS} = ${__TRUE} ] ; then
      LogMsg "All disabled tasks are enabled again"
      LogInfo "The disabled tasks that are now re-enabled are: " &&
        LogMsg "-" "${DISABLED_TASKS}"
    else
      set -f

      EXECUTE_TASK=${__TRUE}
      for CUR_EXCLUDE_TASK_MASK in ${DISABLED_TASKS} ; do
        if [[ ${CUR_TASK#task_*}  == ${CUR_EXCLUDE_TASK_MASK} || ${CUR_TASK} == ${CUR_EXCLUDE_TASK_MASK} || ${CUR_TASK} == ${CUR_EXCLUDE_TASK_MASK#task_*} ]] ; then
          set +f
...
#  LogMsg "-"
  ...
  if [ ${SINGLE_STEP_MODE} = ${__TRUE} -a ${SINGLE_STEP_TEMPORARY_DISABLED} = ${__FALSE} ] ; then
    ...
    while true ; do
      ...
      LogMsg "-" "=================================================================================="
      if [ $i -gt 1 ] ; then
        LogMsg "-" "The last task processed was \"${LAST_TASK_NAME}\" ${LAST_TASK_PARAMETER_MSG} ended with RC=${LAST_TASK_RC}"
      fi
      LogMsg "-"
      AskUser "Execute the task \\\"${CUR_TASK_NAME#task_*}\\\" ${TASK_PARAMETER_MSG} now (qaerlLnY, h for help)?"
      case ${USER_INPUT} in
...
a | all        -- continue and disable single step for the remaining tasks
e | on_error   -- continue and disable single step until a task ends with an error
...
"
        ...
        on_error | e )
          LogMsg "-" "Singlestep mode is now disabled for the remaining tasks until a task ends with an error"
          ENABLE_SINGLE_STEP_ON_ERROR=${__TRUE}
          SINGLE_STEP_MODE=${__FALSE}
          EXECUTE_THIS_STEP=${__TRUE}
          break
  ...
  if ! typeset -f "${CUR_TASK_NAME}" 2>/dev/null 1>/dev/null ; then
    ...
    if [ ${TEMPRC} -eq ${__TRUE} ] ; then
      ...
      if [ ${ENABLE_SINGLE_STEP_ON_ERROR} = ${__TRUE} ] ; then
#
# enable single step on error is true -> enable single step now
#
        SINGLE_STEP_MODE=${__TRUE}
        ENABLE_SINGLE_STEP_ON_ERROR=${__FALSE}

#
# Note: single step on error must be disabled manual in the single step menu
#
        LogMsg "-"
        LogMsg " *** The last task ended with RC=${TEMPRC} - now enabling single step mode"
        LogMsg " *** Note: Enter \"a\" to continue and disable single step mode again, "
        LogMsg " ***       enter \"e\" to continue and disable single step until a task ends with an error"
        LogMsg "-"
      else
